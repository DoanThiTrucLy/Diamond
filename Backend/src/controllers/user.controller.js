...
const cartUser = async (req, res) => {
    try {
        ...
        for (let i = 0; i < cart.cartDiamond.length; i++) {
            const diamond = await Diamond.findById(cart.cartDiamond[i].item);
            cart.cartDiamond[i].item = diamond
        }
        for (let i = 0; i < cart.cartProduct.length; i++) {
            const product = await Product.findById(cart.cartProduct[i].item);
            cart.cartProduct[i].item = product
        }
...
const orderListUser = async(req, res) => {
    try {
        const ordersList = await OrderList.findOne({userID: req.params.id}).populate('userID').populate('orders');
        ...
        for (let i = 0; i < ordersList.orders.length; i++) {
            for (let j = 0; j < ordersList.orders[i].contentDiamond.length; j++) {
                const detail = await Diamond.findById(ordersList.orders[i].contentDiamond[j].itemId);
                ordersList.orders[i].contentDiamond[j].itemId = detail;
            }

            for (let k = 0; k < ordersList.orders[i].contentProduct.length; k++) {
                const detail = await Product.findById(ordersList.orders[i].contentProduct[k].itemId);
