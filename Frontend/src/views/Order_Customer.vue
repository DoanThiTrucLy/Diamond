        ...
        <div class="container ">
            ...
            <div class="col-lg-6  intern">
                <div v-for="(order, index) in orders" :key="index">
                    <div v-for="(diamond, index) in order.contentDiamond" :key="index">
                        <p class="title">{{ diamond.itemId.name }}</p>
                        <span>Số lượng: {{ diamond.quantity }}</span>
                        <p class="price__item">Giá: {{ formattedPrice(diamond.itemId.price) }}đ</p>
                    </div>
                    ...
                    <p>Ngày đặt: {{ order.dateOrder.split('T')[0] }}</p>
...
export default {
    ...
    methods: {
        ...
        sumPricePayment(order) {
            let sum = 0;
            for (let i = 0; i < order.contentDiamond.length; i++) {
                const element = order.contentDiamond[i].itemId.price * order.contentDiamond[i].quantity;
                sum += element;
            }
