                    ...
                    <tbody id="datarow">
                        <tr class="record" v-for="(order, index) in orders" :key="order._id">
                            ...
                            <td style="text-align: left;">
                                <div v-for="(diamond, idx) in order.contentDiamond" :key="idx" v-if="order.contentDiamond.length > 0">
                                    <div>{{ diamond?.itemId?.name }}</div>
                                    <div>Giá: {{ diamond?.itemId?.price }}</div>
                                    <div>Số lượng: {{ diamond?.quantity }}</div>
                                    <br>
                                </div>
    ...
    methods: {
        ...
        async sumPayment() {
            for (let i = 0; i < this.orders.length; i++) {
                ...
                let sum = 0;
                for (let j = 0; j < this.orders[i].contentDiamond.length; j++) {
                    const temp = this.orders[i].contentDiamond[j].itemId.price * this.orders[i].contentDiamond[j].quantity;
                    console.log(this.orders[i].contentDiamond[j].itemId.price);
                    sum += temp;
                }
